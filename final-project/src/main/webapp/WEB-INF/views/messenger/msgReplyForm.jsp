			...
			<div id="msg_list">
				...
				<input type="hidden" name="${_csrf.parameterName}" value="${_csrf.token}"/>
	                <div id="msg_btn_area">
	                    ...
	                    <button type="button" id="reply-reset" class="btn btn-danger">취소하기</button>
	                </div>
	                <!-- 쪽지 작성 영역 -->
	...
	<script>
		...
		$(function(){
			...
			$("#searchSubmit").on("click",function(){
				...
				if(($("#recUser").val().trim()=="") && ($("#recvUno").val().trim()=="")){
				
					// 빈값이라면 str에 담은 userId 맨 마직막에 붙은 구분자 ',' 잘라주기 
					resultStr = str.slice(0,-1);
					resultUno = unoStr.slice(0,-1);
				}else{
					//빈값이 아니라면 
					resultStr = str+$("#recUser").val();
					resultUno = unoStr+$("#recvUno").val();
				}
